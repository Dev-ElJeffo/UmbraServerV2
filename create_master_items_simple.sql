...
INSERT INTO item_templates (
    ...
    'ring',
    1,
    '{"strength":50,"dexterity":50,"intelligence":50,"vitality":50,"luck":50,"health_bonus":500,"mana_bonus":500,"defense":100,"magic_defense":100,"attack":100,"magic_attack":100,"accuracy":50,"dodge":50,"critical":50,"movement":25,"resistance":50,"double_attack_rate":50}',
    'legendary',
    999999,
    0.0
);
...
-- 3. Colar do Mestre (Amulet - usando amulet slot)
INSERT INTO item_templates (
    item_name,
    item_description,
    item_type,
...
-- Função auxiliar para encontrar próximo slot vazio
-- Anel do Mestre
...
SELECT 
    1,
    (SELECT item_id FROM item_templates WHERE item_name = 'Anel do Mestre' ORDER BY item_id DESC LIMIT 1),
    1,
    COALESCE((
...
WHERE NOT EXISTS (
    ...
    AND it.item_name = 'Anel do Mestre'
    AND pi.is_equipped = FALSE
);
...
SELECT 
    1,
    (SELECT item_id FROM item_templates WHERE item_name = 'Amuleto do Mestre' ORDER BY item_id DESC LIMIT 1),
    1,
    COALESCE((
        SELECT MIN(t.slot_index)
        FROM (
...
WHERE NOT EXISTS (
    ...
    INNER JOIN item_templates it ON pi.item_template_id = it.item_id
    WHERE pi.player_id = 1 
    AND it.item_name = 'Amuleto do Mestre'
    AND pi.is_equipped = FALSE
);
...
SELECT 
    1,
    (SELECT item_id FROM item_templates WHERE item_name = 'Colar do Mestre' ORDER BY item_id DESC LIMIT 1),
    1,
    COALESCE((
        SELECT MIN(t.slot_index)
        FROM (
...
SELECT 
    1,
    (SELECT item_id FROM item_templates WHERE item_name = 'Brincos do Mestre' ORDER BY item_id DESC LIMIT 1),
    1,
    COALESCE((
        SELECT MIN(t.slot_index)
        FROM (
