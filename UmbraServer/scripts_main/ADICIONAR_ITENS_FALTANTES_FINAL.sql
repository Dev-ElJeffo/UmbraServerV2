-- ============================================
-- ADICIONAR ITENS FALTANTES AO INVENTÁRIO (VERSÃO FINAL CORRIGIDA)
-- ============================================

-- Encontrar IDs dos itens do Mestre
SET @ring_id = (SELECT item_id FROM item_templates WHERE item_name = 'Anel do Mestre' ORDER BY item_id DESC LIMIT 1);
SET @amulet_id = (SELECT item_id FROM item_templates WHERE item_name = 'Amuleto do Mestre' ORDER BY item_id DESC LIMIT 1);
SET @necklace_id = (SELECT item_id FROM item_templates WHERE item_name = 'Colar do Mestre' ORDER BY item_id DESC LIMIT 1);
SET @earring_id = (SELECT item_id FROM item_templates WHERE item_name = 'Brincos do Mestre' ORDER BY item_id DESC LIMIT 1);

-- Verificar quais já estão no inventário (não equipados)
SET @ring_in_inv = (SELECT COUNT(*) FROM player_inventory pi 
                    INNER JOIN item_templates it ON pi.item_template_id = it.item_id 
                    WHERE pi.player_id = 1 AND it.item_name = 'Anel do Mestre' AND pi.is_equipped = FALSE);
SET @amulet_in_inv = (SELECT COUNT(*) FROM player_inventory pi 
                      INNER JOIN item_templates it ON pi.item_template_id = it.item_id 
                      WHERE pi.player_id = 1 AND it.item_name = 'Amuleto do Mestre' AND pi.is_equipped = FALSE);
SET @necklace_in_inv = (SELECT COUNT(*) FROM player_inventory pi 
                        INNER JOIN item_templates it ON pi.item_template_id = it.item_id 
                        WHERE pi.player_id = 1 AND it.item_name = 'Colar do Mestre' AND pi.is_equipped = FALSE);
SET @earring_in_inv = (SELECT COUNT(*) FROM player_inventory pi 
                       INNER JOIN item_templates it ON pi.item_template_id = it.item_id 
                       WHERE pi.player_id = 1 AND it.item_name = 'Brincos do Mestre' AND pi.is_equipped = FALSE);

-- Colar do Mestre (se não estiver no inventário)
-- Usar uma abordagem mais simples: encontrar o próximo slot vazio diretamente
INSERT INTO player_inventory (
    player_id,
    item_template_id,
    quantity,
    slot_index,
    is_equipped,
    durability,
    custom_properties,
    acquired_at
)
SELECT 
    1,
    @necklace_id,
    1,
    (
        -- Encontrar o primeiro slot vazio (0-49) que não está ocupado
        SELECT t.slot_index
        FROM (
            SELECT 0 as slot_index UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
            UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9
            UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12 UNION ALL SELECT 13 UNION ALL SELECT 14
            UNION ALL SELECT 15 UNION ALL SELECT 16 UNION ALL SELECT 17 UNION ALL SELECT 18 UNION ALL SELECT 19
            UNION ALL SELECT 20 UNION ALL SELECT 21 UNION ALL SELECT 22 UNION ALL SELECT 23 UNION ALL SELECT 24
            UNION ALL SELECT 25 UNION ALL SELECT 26 UNION ALL SELECT 27 UNION ALL SELECT 28 UNION ALL SELECT 29
            UNION ALL SELECT 30 UNION ALL SELECT 31 UNION ALL SELECT 32 UNION ALL SELECT 33 UNION ALL SELECT 34
            UNION ALL SELECT 35 UNION ALL SELECT 36 UNION ALL SELECT 37 UNION ALL SELECT 38 UNION ALL SELECT 39
            UNION ALL SELECT 40 UNION ALL SELECT 41 UNION ALL SELECT 42 UNION ALL SELECT 43 UNION ALL SELECT 44
            UNION ALL SELECT 45 UNION ALL SELECT 46 UNION ALL SELECT 47 UNION ALL SELECT 48 UNION ALL SELECT 49
        ) t
        WHERE t.slot_index NOT IN (
            SELECT pi.slot_index 
            FROM player_inventory pi
            WHERE pi.player_id = 1 
            AND pi.is_equipped = FALSE
        )
        ORDER BY t.slot_index ASC
        LIMIT 1
    ),
    FALSE,
    100.0,
    '[]',
    NOW()
WHERE @necklace_id IS NOT NULL 
  AND @necklace_in_inv = 0
  AND NOT EXISTS (
    SELECT 1 FROM player_inventory pi
    INNER JOIN item_templates it ON pi.item_template_id = it.item_id
    WHERE pi.player_id = 1 
    AND it.item_name = 'Colar do Mestre'
    AND pi.is_equipped = FALSE
  )
  AND EXISTS (
    -- Verificar se há pelo menos um slot vazio
    SELECT 1
    FROM (
        SELECT 0 as slot_index UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
        UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9
        UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12 UNION ALL SELECT 13 UNION ALL SELECT 14
        UNION ALL SELECT 15 UNION ALL SELECT 16 UNION ALL SELECT 17 UNION ALL SELECT 18 UNION ALL SELECT 19
        UNION ALL SELECT 20 UNION ALL SELECT 21 UNION ALL SELECT 22 UNION ALL SELECT 23 UNION ALL SELECT 24
        UNION ALL SELECT 25 UNION ALL SELECT 26 UNION ALL SELECT 27 UNION ALL SELECT 28 UNION ALL SELECT 29
        UNION ALL SELECT 30 UNION ALL SELECT 31 UNION ALL SELECT 32 UNION ALL SELECT 33 UNION ALL SELECT 34
        UNION ALL SELECT 35 UNION ALL SELECT 36 UNION ALL SELECT 37 UNION ALL SELECT 38 UNION ALL SELECT 39
        UNION ALL SELECT 40 UNION ALL SELECT 41 UNION ALL SELECT 42 UNION ALL SELECT 43 UNION ALL SELECT 44
        UNION ALL SELECT 45 UNION ALL SELECT 46 UNION ALL SELECT 47 UNION ALL SELECT 48 UNION ALL SELECT 49
    ) t
    WHERE t.slot_index NOT IN (
        SELECT pi.slot_index 
        FROM player_inventory pi
        WHERE pi.player_id = 1 
        AND pi.is_equipped = FALSE
    )
    LIMIT 1
  );

-- Brincos do Mestre (se não estiver no inventário)
INSERT INTO player_inventory (
    player_id,
    item_template_id,
    quantity,
    slot_index,
    is_equipped,
    durability,
    custom_properties,
    acquired_at
)
SELECT 
    1,
    @earring_id,
    1,
    (
        -- Encontrar o primeiro slot vazio (0-49) que não está ocupado
        SELECT t.slot_index
        FROM (
            SELECT 0 as slot_index UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
            UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9
            UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12 UNION ALL SELECT 13 UNION ALL SELECT 14
            UNION ALL SELECT 15 UNION ALL SELECT 16 UNION ALL SELECT 17 UNION ALL SELECT 18 UNION ALL SELECT 19
            UNION ALL SELECT 20 UNION ALL SELECT 21 UNION ALL SELECT 22 UNION ALL SELECT 23 UNION ALL SELECT 24
            UNION ALL SELECT 25 UNION ALL SELECT 26 UNION ALL SELECT 27 UNION ALL SELECT 28 UNION ALL SELECT 29
            UNION ALL SELECT 30 UNION ALL SELECT 31 UNION ALL SELECT 32 UNION ALL SELECT 33 UNION ALL SELECT 34
            UNION ALL SELECT 35 UNION ALL SELECT 36 UNION ALL SELECT 37 UNION ALL SELECT 38 UNION ALL SELECT 39
            UNION ALL SELECT 40 UNION ALL SELECT 41 UNION ALL SELECT 42 UNION ALL SELECT 43 UNION ALL SELECT 44
            UNION ALL SELECT 45 UNION ALL SELECT 46 UNION ALL SELECT 47 UNION ALL SELECT 48 UNION ALL SELECT 49
        ) t
        WHERE t.slot_index NOT IN (
            SELECT pi.slot_index 
            FROM player_inventory pi
            WHERE pi.player_id = 1 
            AND pi.is_equipped = FALSE
        )
        ORDER BY t.slot_index ASC
        LIMIT 1
    ),
    FALSE,
    100.0,
    '[]',
    NOW()
WHERE @earring_id IS NOT NULL 
  AND @earring_in_inv = 0
  AND NOT EXISTS (
    SELECT 1 FROM player_inventory pi
    INNER JOIN item_templates it ON pi.item_template_id = it.item_id
    WHERE pi.player_id = 1 
    AND it.item_name = 'Brincos do Mestre'
    AND pi.is_equipped = FALSE
  )
  AND EXISTS (
    -- Verificar se há pelo menos um slot vazio
    SELECT 1
    FROM (
        SELECT 0 as slot_index UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
        UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9
        UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12 UNION ALL SELECT 13 UNION ALL SELECT 14
        UNION ALL SELECT 15 UNION ALL SELECT 16 UNION ALL SELECT 17 UNION ALL SELECT 18 UNION ALL SELECT 19
        UNION ALL SELECT 20 UNION ALL SELECT 21 UNION ALL SELECT 22 UNION ALL SELECT 23 UNION ALL SELECT 24
        UNION ALL SELECT 25 UNION ALL SELECT 26 UNION ALL SELECT 27 UNION ALL SELECT 28 UNION ALL SELECT 29
        UNION ALL SELECT 30 UNION ALL SELECT 31 UNION ALL SELECT 32 UNION ALL SELECT 33 UNION ALL SELECT 34
        UNION ALL SELECT 35 UNION ALL SELECT 36 UNION ALL SELECT 37 UNION ALL SELECT 38 UNION ALL SELECT 39
        UNION ALL SELECT 40 UNION ALL SELECT 41 UNION ALL SELECT 42 UNION ALL SELECT 43 UNION ALL SELECT 44
        UNION ALL SELECT 45 UNION ALL SELECT 46 UNION ALL SELECT 47 UNION ALL SELECT 48 UNION ALL SELECT 49
    ) t
    WHERE t.slot_index NOT IN (
        SELECT pi.slot_index 
        FROM player_inventory pi
        WHERE pi.player_id = 1 
        AND pi.is_equipped = FALSE
    )
    LIMIT 1
  );

-- Verificar resultado final
SELECT 
    it.item_id,
    it.item_name,
    it.equipment_slot,
    pi.inventory_id,
    pi.slot_index,
    pi.is_equipped
FROM item_templates it
LEFT JOIN player_inventory pi ON pi.item_template_id = it.item_id AND pi.player_id = 1
WHERE it.item_name LIKE '%Mestre%'
ORDER BY it.item_id;

