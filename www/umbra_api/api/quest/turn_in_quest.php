...
try {
    ...
    $result = questTurnIn($pdo, $player_id, $quest_id, $npc_template_id);
    if (!$result['ok']) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => $result['message'] ?? 'Falha no turn-in.']);
        exit;
    }
    echo json_encode(array_merge(['success' => true], $result), JSON_UNESCAPED_UNICODE);
} catch (PDOException $e) {
    error_log('turn_in_quest: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Erro ao entregar quest.']);
}
