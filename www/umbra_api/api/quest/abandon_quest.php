...
try {
    ...
    $result = questAbandon($pdo, $player_id, $quest_id);
    if (!$result['ok']) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => $result['message'] ?? 'Falha ao abandonar.']);
        exit;
    }
    echo json_encode(['success' => true, 'quest_id' => $quest_id], JSON_UNESCAPED_UNICODE);
} catch (PDOException $e) {
    error_log('abandon_quest: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Erro ao abandonar quest.']);
}
