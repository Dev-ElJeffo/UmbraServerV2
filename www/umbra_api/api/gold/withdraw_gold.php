<?php
/**
 * POST /api/gold/withdraw_gold.php
 * Saca gold do armazém da conta para o inventário do jogador
 * 
 * Body (JSON):
 * {
 *   "token": "jwt_token",
...
try {
    ...
    // Buscar gold armazenado atual
    $account_query = "SELECT stored_gold FROM accounts WHERE id = :account_id FOR UPDATE";
    ...
    $current_stored_gold = (int)($account_result['stored_gold'] ?? 0);
    
    // Verificar se tem gold suficiente no armazém
    if ($current_stored_gold < $amount) {
        $pdo->rollBack();
        http_response_code(400);
        echo json_encode([
            'success' => false, 
            'message' => 'Gold insuficiente no armazém',
            'stored_gold' => $current_stored_gold,
            'requested_amount' => $amount
        ]);
        exit;
    }
    
    // Subtrair gold do armazém
    ...
    echo json_encode([
        'success' => true,
        'message' => "Sacado $amount gold do armazém",
    ...
    error_log("Erro ao sacar gold: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
        'message' => 'Erro ao sacar gold',
        'error' => $e->getMessage()
    ]);
}
?>
